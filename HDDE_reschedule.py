...
global main_info
main_info = [[11,4,6],[4,5,25],[9,20,45]]
...
def perform_scheduling_origin(jobs, unit_assignment_vectors, order_sequence_vectors, num_orders, num_stages,
                       out_schedule=False):
...
def perform_scheduling(jobs, unit_assignment_vectors, order_sequence_vectors, num_orders, num_stages,
                       maintenance_info=main_info, out_schedule=False):
    # Perform the scheduling to get the Gantt chart
    machine_schedules = {}  # Record the task list for each machine
    job_completion_times = [0 for _ in range(num_orders)]  # Record the completion time for each order
    CT_ALL = np.zeros(shape=[num_orders, num_stages])  # Record the completion time for each operation
    machine_schedules_mas = []  # Record the task list for each machine
    machine_available_time = {}  # Record the available time for each machine

    # Process maintenance info to create machine maintenance intervals
    maintenance_intervals = {}  # Maintenance intervals per machine
    for maintenance in maintenance_info:
        machine, start, end = maintenance
        if machine not in maintenance_intervals:
            maintenance_intervals[machine] = []
        maintenance_intervals[machine].append((start, end))
    ...
    for stage in range(num_stages):
        ...
        for order_index, _ in sorted_orders_stage:
            ...
            machine_schedule = machine_schedules.get(assigned_machine, [])
            last_machine_end_time = machine_schedule[-1]['Finish'] if machine_schedule else 0
            machine_available = machine_available_time.get(assigned_machine, 0)
            start_time = max(last_machine_end_time, job_completion_times[order_index], machine_available)

            # Check for maintenance periods and adjust start time if necessary
            if assigned_machine in maintenance_intervals:
                for maintenance_start, maintenance_end in maintenance_intervals[assigned_machine]:
                    if start_time < maintenance_start < start_time + processing_time:
                        # Maintenance starts during the operation
                        # start_time = start_time
                        end_time = start_time + (maintenance_end - maintenance_start) + processing_time
                    elif maintenance_start <= start_time < maintenance_end:
                        # Operation starts during maintenance
...
def read_best_solution(file_path):
    """
    读取best_solution.txt文件并将其内容转换为字典。
    """
    try:
        with open(file_path, 'r', encoding='utf-8') as file:
            content = file.read()
        # 使用ast.literal_eval安全地解析字符串为字典
        best_solution = ast.literal_eval(content)

        # 转换unit_assignment中的字符串为整数
        if 'unit_assignment' in best_solution:
            for i in range(len(best_solution['unit_assignment'])):
                for j in range(len(best_solution['unit_assignment'][i])):
                    value = best_solution['unit_assignment'][i][j]
                    if isinstance(value, str):
                        if value.lower() == 'none':
                            best_solution['unit_assignment'][i][j] = None
                        else:
                            try:
                                best_solution['unit_assignment'][i][j] = int(value)
                            except ValueError:
                                best_solution['unit_assignment'][i][j] = None
        ...
        print(f"文件内容格式错误，无法解析为字典: {file_path}")
...
if __name__ == "__main__":
    ...
    best_solution_static = read_best_solution('best_solution.txt')
    Ind_one.order_sequence_vectors = best_solution_static['order_sequence']
    Ind_one.unit_assignment_vectors = best_solution_static['unit_assignment']

    Ind_one.decode()
    Ind_one.draw(name=['Init', f'Static{file_name[-5]}_{POPULATION_SIZE}_gen{ITERATIONS}_sd{seed}'], folder='result_ga',
                 maintenance_info=main_info, format_p="svg")
    # run
    ...
    cis = perform_scheduling(jobs, best_solution['unit_assignment'], best_solution['order_sequence'], Ind_one.num_orders,
                             Ind_one.num_stages)
    print(cis)
    Ind_one.draw(name=['HDDE', f'Dyn_{file_name[-5]}_{POPULATION_SIZE}_gen{ITERATIONS}_sd{seed}_t{calcu_time:.1f}'],
                 folder='result_ga', format_p="svg", maintenance_info=main_info)
